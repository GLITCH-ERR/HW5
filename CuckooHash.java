	  ...
	  */
	  ...
	  public void put(K key, V value) {
		...
		if ((table[pos1] != null && table[pos1].getBucKey().equals(key) && table[pos1].getValue().equals(value)) ||
			...
			return; // Key-value pair already present, do nothing
		}

		Bucket<K, V> newBucket = new Bucket<>(key, value);
		
		for (int i = 0; i < CAPACITY; i++) {
			// Try to place in hash1 location
			if (table[pos1] == null) {
				table[pos1] = newBucket;
				return;
			}
			
			// Evict current element
			Bucket<K, V> displaced = table[pos1];
			table[pos1] = newBucket;
			
			// Move evicted element to its alternate location
			newBucket = displaced;
			pos1 = (pos1 == hash1(displaced.getBucKey())) ? hash2(displaced.getBucKey()) : hash1(displaced.getBucKey());
		...
		// If we reach here, we detected a cycle and must rehash
		rehash();
		put(newBucket.getBucKey(), newBucket.getValue()); // Retry insertion after rehashing
	 }
